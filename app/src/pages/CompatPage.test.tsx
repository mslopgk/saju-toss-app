...
describe('CompatPage — 상대방 입력 화면', () => {
  it('던지지 않고, 첫 사람을 다시 묻지 않는다', () => {
    ...
    /*
      "첫 사람을 다시 묻지 않는다" 를 **문장이 아니라 구조로** 본다.

      예전에는 부제("내 정보는 이미 받았으니 다시 묻지 않아요")가 있는지를 봤는데, 그건
      화면이 그 약속을 지킨다는 증거가 아니라 그렇게 적혀 있다는 증거일 뿐이다.
      부제를 지우자 이 검사가 깨졌고 — 약속은 그대로인데 검사만 깨졌다는 뜻이었다.

      실제 증거는 이 화면에 **온보딩의 제출 경로가 없다**는 것이다.
    */
    expect(html).not.toContain('결과 보기')
    expect(html).toContain('궁합 보기')
  })
