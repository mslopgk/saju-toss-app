 ...
 * 그래서 픽셀을 직접 만든다.
 *
 * 십이지·궁합도 대상에 넣었다. 처음에는 "작게 쓰여 얼룩이 눈에 띄지 않는다" 고 판단해 빼
 * 뒀는데, 네 기둥 표를 실제로 찍어 보니 60px 타일에서도 **둥근 네모가 또렷했다** — 오행에서
 * 고친 것과 같은 문제였다. 궁합을 뺀 이유("두 사람 사이 거리가 정보라 배경을 지우면 여백이
 * 사라진다")도 틀렸다: 배경이 투명해질 뿐 두 형상의 간격은 그대로 남는다.
 *
 * 별자리·십신은 아직 화면에서 쓰지 않아 대상이 아니다. 쓰기 시작하면 여기 이름을 더한다.
 */
const KEYED_SETS = new Set(['elements', 'loading', 'branch', 'compat']);
